...
ambientLight(102, 102, 102);
  ...
  table= loadTable( "data/new.csv", "header");
    ...
    fill(0);text("Fertig!",820,160);}
       ...
       fill(0);text("Link",1150,38);} if (rg==1){
     
      fill(0,255,0); circle(1170, 30, 45);fill(0);text("Aktiv",1150,38);}
      if (rg==2){
...
// table= loadTable( "data/new.csv");
  ...
  while (food3!=0) {
    ...
    if (home3.size()<=0)
      home3.append((int)random(0, feld*feld*feld));
    if (food3==home3.get(0)) {
      pushMatrix();
      translate(100+karte*axkarte, 250+karte*aykarte, karte*azkarte );
